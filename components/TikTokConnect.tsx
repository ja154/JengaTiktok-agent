...
const TikTokConnect: React.FC<TikTokConnectProps> = ({ apiKey, setApiKey }) => {
  const isConnected = apiKey.trim() !== '';

  return (
    <div className="bg-slate-800/50 rounded-xl p-4 shadow-lg border border-slate-700 flex flex-col md:flex-row items-center justify-between gap-4">
      <div className="flex items-center gap-3">
        <svg height="24" viewBox="0 0 24 24" width="24" xmlns="http://www.w3.org/2000/svg" className="text-cyan-400 flex-shrink-0">
        ...
        <div>
            ...
            <p className="text-sm text-slate-400">
                {isConnected ? `API Key is set and ready.` : "Provide your API key to enable posting."}
            </p>
        </div>
      </div>
      <div className="w-full md:w-auto flex-grow md:max-w-xs">
          <input
            type="password"
            placeholder="Enter your TikTok API Key here"
            value={apiKey}
            onChange={(e) => setApiKey(e.target.value)}
            className="w-full bg-slate-900 border border-slate-600 rounded-lg p-2 focus:ring-2 focus:ring-purple-500 focus:border-purple-500 transition-all duration-300 placeholder-slate-500 text-sm"
          />
      </div>
    </div>
  );
